...
parser.add_argument("--data-aug-affine", default = 0, type=int)
...
def main(args):
    composeArgs = [torchvision.transforms.ToTensor(),]
    useComposeArgs = 0
    if(args.data_aug_hflip is True):
        useComposeArgs = 1
        composeArgs.insert(0, torchvision.transforms.RandomHorizontalFlip())
    if(args.data_aug_brightness):
        useComposeArgs = 1
        changeBrightness = args.data_aug_brightness
        composeArgs.insert(0, torchvision.transforms.ColorJitter(brightness = changeBrightness))
    if(args.data_aug_affine):
        useComposeArgs = 1
        changeAffine = args.data_aug_affine
        composeArgs.insert(0, torchvision.transforms.RandomAffine(degrees=changeAffine))
    trainTransform = transforms.ToTensor()
    transform = transforms.ToTensor()
    if(useComposeArgs):
        trainTransform = Compose(composeArgs)
    args.dataset_root.mkdir(parents=True, exist_ok=True)
    train_dataset = torchvision.datasets.CIFAR10(
        args.dataset_root, train=True, download=True, transform=trainTransform
    )
...
def get_summary_writer_log_dir(args: argparse.Namespace) -> str:
    ...
    tb_log_dir_prefix = (
      ...
      f"affine={args.data_aug_affine}_" +
